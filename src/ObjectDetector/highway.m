...
if readImages == 1
    ...
    imagefiles = dir('../src/data/highway/*.jpg');
    ...
    parfor i=1:length(imagefiles)
        rgbimages(:,:,:,i) = (imread(strcat('../src/data/highway/',imagefiles(i).name)));
...
if findObjects == 1
    ...
    for i=2:length(imagefiles)
        ...
        noiseRemovedImage = imerode(thresholdedImage, ones(4,4));
        dilatedImage = imdilate(noiseRemovedImage, ones(8,8));
...
movie = immovie(outRGBimages);
movie2avi(movie, 'high.avi');

% for i=1:length(imagefiles)
%     imwrite(outRGBimages(:,:,:,i), strcat('../src/data/out9-highway/out', sprintf('%06d', i), '.jpg'));
% end
